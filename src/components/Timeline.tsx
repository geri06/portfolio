  ...
  return (
    <div className={`relative flex items-start justify-between md:justify-normal w-full ${isEven ? 'md:flex-row-reverse' : 'md:flex-row'}`} ref={ref} style={{ marginBottom: '2rem' }}>
      ...
      >
        ...
        <h3 className="text-2xl font-bold text-[#1d1d1f]" style={{ marginBottom: '0.6rem' }}>{item.title}</h3>
        <h4 className="text-lg text-[#86868b]" style={{ marginBottom: '1rem' }}>{item.company}</h4>
...
export const Timeline = () => {
  return (
    <section id="experience" className="px-8 md:px-16 relative" style={{ paddingTop: '3rem', paddingBottom: '3rem' }}>
      <div className="section-container flex flex-col items-center">
        <div className="text-center" style={{ marginBottom: '4rem' }}>
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h2 className="text-4xl md:text-6xl font-bold text-[#1d1d1f]" style={{ marginBottom: '1rem' }}>Professional Experience</h2>
            <p className="text-xl text-[#86868b] max-w-2xl mx-auto">My journey in AI, Robotics, and Data Science</p>
          </motion.div>
        </div>
